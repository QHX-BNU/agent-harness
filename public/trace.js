...
(function () {
  ...
  function summarize(ev) {
    ...
    const map = {
      ...
      memory_profile: () => `常驻画像注入 ${ev.chars || 0} 字符${ev.truncated?.length ? `（截断 ${ev.truncated.join(',')}）` : ''}`,
      memory: () => `${ev.action} #${ev.item?.id} ${cut(ev.item?.content, 80)}`,
      skills: () => `技能清单 ${ev.count || 0} 个${(ev.names || []).length ? `：${(ev.names || []).join(', ')}` : ''}`,
      skill: () => `${ev.action} ${ev.name}${ev.description ? ` · ${cut(ev.description, 60)}` : ''}`,
