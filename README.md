...
会话里的每条用户消息都带**结构化说话人**（谁、哪个群、哪个话题、什么时候），
所以「用户在 A 群，想知道同事在 B 群干了什么」是可以回答的：

| 工具 | 回答什么问题 |
|---|---|
| `user_activity(user, hours)` | 「小明最近做了什么」——他在哪些群/话题出现过、提了什么要求、得到什么结论、留下什么任务 |
| `session_list(user)` | 某人参与过的会话清单（含参与者与最后结论摘要） |
| `session_read(session)` | 读某个会话的细节，消息按 `【姓名】` 标注 |

实测（真实模型）：

```
爱丽丝 在「oc_team_a」说: @机器人 小明最近做了什么？他在别的群里干了啥？

→ 小明最近（168 小时内）只在 1 个会话出现过：
  - 飞书群「team_b」（约 1 分钟前）：让机器人往 docs/TODO.md 追加一条待办
    「补充单元测试」，已追加到文件末尾。不过文件里第 4 行本来就有「加单测」，
    机器人提醒可能与新加的第 5 行重复，问要不要合并——这个还没定。
```

**隐私边界**：这套能力默认开启，前提是所有群在**同一个工作区**。它意味着
任意群里的人都能查到本工作区其他会话里谁说了什么。需要更严的隔离时：
...
```
挡住 12 / 15，绕过 3
  ✗ 把 payload 写进文件再执行     ← 命令行干净，文件内容不经过扫描
  ✗ base64 编码的命令            ← EncodedCommand 解出来才看得见
...
run.cmd --jail --allow-shell
```

`--jail` 做的事：把**代码目录设为只读白名单、工作区与数据目录设为可写白名单**，然后带
`--permission` 启动。实测结论（`scripts/jail-test.js`，13 项）：

```
✓ 隔离等级自报为 runtime（不撒谎）        ✓ 工作区内读/写正常
✓ 工作区外读被拒                        ✓ 上表那三招全部被运行时挡住
...
| `off` | 完全禁网 |
| `whitelist` | 只允许列表里的主机 |
| `blacklist` | 禁止列表里的主机，其余放行 |

```powershell
$env:SANDBOX_NETWORK="whitelist"
$env:SANDBOX_NETWORK_LIST="api.github.com,*.npmjs.org,localhost:8080"
```

界面上是 设置 → 沙箱 → 网络访问（模式 + 主机列表 + 「测一下」即时试规则）。
主机规则支持 `example.com`、`*.example.com`（含根域）、`host:port`、IP 字面量、`*`。

**两层执行**，不是只做正则：

1. **命令分析**（事前）：从命令行抠出目标主机，白名单模式下目标不明就直接拒绝（宁可拦错）
2. **本地代理**（事中）：给子进程注入 `HTTP(S)_PROXY`，指向一个只认这次策略的本地代理
...
> 边界说清楚：代理是**合作式**的。程序如果自己实现网络栈、无视代理变量直连，仍能出去；
> 要绝对禁止只能用容器级隔离。所以网络策略是「大幅收窄 + 可审计」，配合 `--jail` 或容器一起用最稳。
